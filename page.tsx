...
const MotionWrapper = ({ children, delay = 0 }: MotionWrapperProps) => {
  return (<motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay }}>{children}</motion.div>)
...
export default function Home() {
  ...
  return (
    <div className='min-h-screen'>
      <header className='container mx-auto px-4 py-6 flex justifiy-betweem items-center'>
        ...
        <section className='mb-24'>
          <div className='flex flex-col justify-between md:flex-row items-center'>
            <div className='md:w-1/2 mb-8 md:mb-0'>
              <MotionWrapper>
                <h2 className='text-4xl md:text-6xl font-bold'>discover gadgets on the go</h2>
              </MotionWrapper>
              <MotionWrapper delay={0.2}>
                <h2 className='text-xl mb-6'>shop the latest tech rigth from your pocket with an innovative mobile app</h2>
              </MotionWrapper>
              <MotionWrapper delay={0.4}>
                <Button size='lg' className='bg-[#1BC464]'>download now <ChevronRight className='ml-2 h-4 v-4'></ChevronRight></Button>
              </MotionWrapper>
            </div>
            <div className='w-1/3'><motion.div className='relative' style={{ perspective: 1000 }} animate={{
              rotateX: isHovered ? rotate.rotateX : 0,
              rotateY: isHovered ? rotate.rotateX : 0
            }} transition={{ type: 'spring', stiffness: 400, damping: 15 }} onMouseMove={handleMouseMove} onHoverStart={() => setIsHovered(true)} onHoverEnd={() => setIsHovered(false)}>
              <Image width={1332} height={888} src='/app-pics.png' alt='GadgetApp screenshots' className='rounded-3xl object-cover shadow-2xl mx-auto h-[500px] w-[250px]'></Image>
